...
{	
	...
	public function deleteAction($videoid)
	{
		...
		if($video == null || $video->getChannel() !== $user->getChannel())
		{
			...
			if(file_exists($video->getLocation().'/../'.$video->getId().'.png'))
			{
				unlink($video->getLocation().'/../'.$video->getId().'.png');
			}
	...
	public function serveruploadAction()
	{
		...
		if($channel->getS3bucket()) // S3 Upload
		{
			...
			$uploadinfo['s3params'] = $this->getS3UploadParam($channel->getS3bucket(), $returnpath, strval(time()).'/', !$noflash);
		}
		else // host on own server
		{
			...
			if ($this->getRequest()->getMethod() === 'POST') {
				...
				else 
				{
					...
					$this->createThumb($video);
					return $this->redirect($this->generateUrl('video_list'));
				}
			}
		}
		
		if($noflash){return $this->render('FlouVideoManagerBundle:Video:serverupload.html.twig', array(
		...
		if ($this->getRequest()->getMethod() === 'GET') {
			...
			if($bucket && $key) //valid get
			{
				...
				if(preg_match('/\.(mp4|m4v|f4v|webm|flv)$/i', $key))
				{
					...
					$video->setHttp($channel->getDownloaddistribution().'/'.$key); // this is not the same location as in our database
	...
	protected function createThumb($video)
	{
		...
		$statement = $this->container->getParameter('ffmpeg.dir').' -i '.$video->getHttp().' -itsoffset 4 -vcodec png -vframes 1 -an -f rawvideo -s 272x153 -ss 0:0:5 -y "'.$video->getLocation().'/../'.$video->getId().'.png"';
