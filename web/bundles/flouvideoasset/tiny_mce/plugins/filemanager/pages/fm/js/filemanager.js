(function($){
	window.FileManager = $.extend(BaseManager, {
		...
		init : function() {
			...
			$().bind('action:selectall', function() {
				if (t.files.length && !t.focusedFile) {
					if (t.files[0].type == "parent" && t.files.length > 1) {
						t.focusFile(t.files[1]);
					} else {
						t.focusFile(t.files[0]);						
					}
				}

				t.setFileSelection(0, t.files.length, true);
			});
				...
				setup : function(m) {
					$(m).bind('DropMenu:beforeshow', function(e, m) {
						function isVisible(action) {
							return $.inArray(action, t.visible) != -1;
						};

						// Build new menu
						m.clear();
						isVisible('cut') && m.add({title : $.translate('{#actions.cut}'), disabled : t.isDisabled('cut') || !t.selectedFiles.length, onclick : function() {t.setClipBoard({files : t.selectedFiles, action : 'cut'});}});
						isVisible('copy') && m.add({title : $.translate('{#actions.copy}'), disabled : t.isDisabled('copy') || !t.selectedFiles.length, onclick : function() {t.setClipBoard({files : t.selectedFiles, action : 'copy'});}});
						isVisible('paste') && m.add({title : $.translate('{#actions.paste}'), disabled : t.isDisabled('paste') || !t.clipboard, onclick : function() {t.pasteFiles();}});

						if (m.items.length)
							m.addSeparator();

						isVisible('edit') && m.add({title : $.translate('{#actions.edit}'), disabled : !t.focusedFile || (!t.focusedFile.custom || !t.focusedFile.custom.editable) || t.isDisabled('edit') || !t.selectedFiles.length, onclick : function() {t.editFile(t.focusedFile);}});
						isVisible('rename') && m.add({title : $.translate('{#actions.rename}'), disabled : t.isDisabled('rename') || !t.selectedFiles.length, onclick : function() {t.renameFile(t.focusedFile);}});
						isVisible('delete') && m.add({title : $.translate('{#actions.deleteit}'), disabled : t.isDisabled('delete') || !t.selectedFiles.length, onclick : function() {t.deleteFiles(t.selectedFiles);}});
						isVisible('zip') && m.add({title : $.translate('{#actions.zip}'), disabled : t.isDisabled('zip') || !t.selectedFiles.length, onclick : function() {t.zipFiles();}});
					});
				}
			});
		...
		showPreview : function(fi) {
			if (fi && (!fi.custom || fi.custom.previewable) && fi.type != 'zip')
				...
				$('#preview').attr('src', "about:blank").css('visibility', 'hidden');
			}
		},
